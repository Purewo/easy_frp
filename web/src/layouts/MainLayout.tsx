import { Layout, Menu, Typography, Avatar, Badge, Space, Divider, Tooltip } from 'antd';
import {
  DatabaseOutlined,
  DesktopOutlined,
  FileTextOutlined,
...
const menuItems = [
  ...
  { key: '/rooms', label: 'XTCP 房间', icon: <ShareAltOutlined /> },
...
const pageMeta: Record<string, { title: string; subtitle: string; icon: ReactNode }> = {
  ...
  '/rooms': { title: 'XTCP 房间', subtitle: '管理 NAT 穿透房间与主机/访问端交接', icon: <ShareAltOutlined /> },
...
export default function MainLayout() {
  ...
  return (
    <Layout style={{ minHeight: '100vh' }}>
      <Sider theme="dark" collapsible width={240} onCollapse={setCollapsed} style={{ padding: '24px 0 0' }}>
        <div style={{ padding: collapsed ? '0 0 20px' : '0 20px 20px', display: 'flex', alignItems: 'center', justifyContent: collapsed ? 'center' : 'flex-start', gap: 12 }}>
          <Tooltip title="FRP 管理器" placement="right" mouseEnterDelay={collapsed ? 0.2 : 999}>
            <div
              style={{
                width: 36,
                height: 36,
        ...
        {!collapsed && <Divider style={{ margin: '0 16px 12px', borderColor: 'rgba(255,255,255,0.06)' }} />}

        <div style={{ padding: '0 12px 16px' }}>
          <Tooltip title="本地客户端：直连 frpc 端口暴露" placement="right" mouseEnterDelay={collapsed ? 0.2 : 999}>
            <div
              style={{
                display: 'flex',
                alignItems: 'center',
